...
Przeplyw: `/admin/` login (`#id_username`/`#id_password`) na adresie panelu z pola konta PartnerTax (`account.fields.baseUrl`, "Adres panelu PartnerTax" w GUI) → `/admin/finances/reckoning/` → pierwszy wiersz z "Finished" = False → w formularzu zmiany: "Add another Data source" → wypelnij `select[name="sources-N-system"]`, `select[name="sources-N-city"]` i `select[name="sources-N-company"]` - ID tych opcji sa odczytywane w czasie dzialania z szablonu formsetu (ukryty wiersz `select[name="sources-__prefix__-*"]`) i dopasowywane po tekscie (dokladny tekst opcji > normalizacja; dla Company dodatkowo dopasowanie "zawiera", gdy jednoznaczne; niejednoznacznosc = blad z lista opcji) → `input[type=file][name="sources-N-file"]` → **"Save and continue editing" po KAZDYM pliku** (klient podkreslil to wprost - bez tego raport sie nie zapisuje).

**Niezweryfikowane na zywo (TODO):**
- ID System/City/Company juz nie sa hardkodowane - dopasowanie po tekscie opcji (`optionMatching.js`) dziala dla dowolnego konta PartnerTax bez edycji kodu; `SYSTEM_LABEL_CANDIDATES` w `partnertax.js` to tylko kandydaci etykiet System (np. "Bolt" vs "BOLT").
